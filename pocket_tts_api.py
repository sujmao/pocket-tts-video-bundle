...
# Ensure workspace directory is on sys.path (needed for bundled portable Python)
...
# Voice metadata & video generator
from voice_metadata import enrich_voice_list
...
# [OFFLINE] Additional imports for local offline model loading
if POCKET_TTS_AVAILABLE:
    try:
        import pocket_tts.utils.utils as tts_utils
        from pocket_tts.utils.config import load_config as load_tts_config

        OFFLINE_UTILS_AVAILABLE = True
    except ImportError as e:
        OFFLINE_UTILS_AVAILABLE = False
        print(f"[WARNING] pocket_tts internal utils not accessible: {e}")
else:
    OFFLINE_UTILS_AVAILABLE = False
...
# [OFFLINE] Model path constants
MODELS_DIR = Path("models")
LOCAL_WEIGHTS = MODELS_DIR / "model.safetensors"
LOCAL_TOKENIZER = MODELS_DIR / "tokenizer.model"
LOCAL_EMBEDDINGS_DIR = MODELS_DIR / "embeddings"
...
if POCKET_TTS_AVAILABLE:
    try:
        ...
        # [OFFLINE] Check if local pre-downloaded models exist
        if LOCAL_WEIGHTS.exists() and LOCAL_TOKENIZER.exists():
            print("[INFO] Local models found — loading in OFFLINE mode")

            # Locate the built-in english.yaml config from the installed pocket_tts package
            import pocket_tts

            config_path = Path(pocket_tts.__file__).parent / "config" / "english.yaml"
            if not config_path.exists():
                # Fallback: try to find it relative to the package
                config_path = (
                    Path(pocket_tts.__file__).parent.parent / "config" / "english.yaml"
                )
            print(f"[INFO] Loading config from: {config_path}")
            tts_config = load_tts_config(config_path)

            # Override the 3 hf:// paths to local files
            tts_config.weights_path = str(LOCAL_WEIGHTS.resolve())
            tts_config.weights_path_without_voice_cloning = str(LOCAL_WEIGHTS.resolve())
            tts_config.flow_lm.lookup_table.tokenizer_path = str(
                LOCAL_TOKENIZER.resolve()
            )
            print(f"[INFO] Config paths overridden to local models/ directory")

            # Load model using internal constructor (bypasses HuggingFace download)
            # origin=config_path is REQUIRED — without it, predefined voices fail
            # because get_state_for_audio_prompt() checks self.origin.is_relative_to(CONFIGS_DIR)
            tts_model = TTSModel._from_pydantic_config_with_weights(
                ...
                None,  # noise_clamp
                -4.0,  # eos_threshold
                config_path,  # origin — required for predefined voice support
            )
            print("[INFO] TTS model loaded from local files (offline mode)")

            # [OFFLINE] Monkey-patch get_predefined_voice to return local embedding files
            # (pocket_tts >=2.0 uses safetensors embeddings via get_predefined_voice,
            #  not the PREDEFINED_VOICES dict which no longer exists.)
            if LOCAL_EMBEDDINGS_DIR.exists():
                _original_get_predefined_voice = tts_utils.get_predefined_voice
                overridden_count = 0

                def _make_offline_voice_fn(embeddings_dir, original_fn):
                    def _offline_get_predefined_voice(language, name):
                        local_path = embeddings_dir / f"{name}.safetensors"
                        if local_path.exists():
                            return str(local_path.resolve())
                        # Fallback to original (triggers online download if available)
                        return original_fn(language, name)
                    return _offline_get_predefined_voice

                tts_utils.get_predefined_voice = _make_offline_voice_fn(
                    LOCAL_EMBEDDINGS_DIR, _original_get_predefined_voice
                )

                for voice_name in tts_utils._ORIGINS_OF_PREDEFINED_VOICES:
                    if (LOCAL_EMBEDDINGS_DIR / f"{voice_name}.safetensors").exists():
                        overridden_count += 1
                print(
                    f"[INFO] {overridden_count} predefined voices mapped to local embeddings"
                )
            else:
                print("[INFO] No local embeddings directory found — skipping voice override")
        else:
            # Fallback: normal online mode (for build machine before model download)
            print("[INFO] Local models not found — loading from default (online)")
...
def scan_voices():
    ...
    # [OFFLINE] Add built-in predefined voices whose embedding files actually exist
    if POCKET_TTS_AVAILABLE:
        try:
            added = 0
            for voice_name in tts_utils._ORIGINS_OF_PREDEFINED_VOICES:
                voice_id = voice_name.lower().replace(" ", "-").replace("_", "-")
                # Skip if already present from WAV scan
                if any(v["voice_id"] == voice_id for v in voices):
                    continue
                # Only add if the embedding file actually exists locally
                embedding_path = LOCAL_EMBEDDINGS_DIR / f"{voice_name}.safetensors"
                if embedding_path.exists():
                    ...
                    added += 1
            print(f"[INFO] Added {added} built-in voices (embeddings verified)")
        except Exception as e:
            print(f"[WARNING] Failed to enumerate built-in voices: {e}")
...
def get_voice_state(voice_id):
    ...
    if voice_id in available_voices and tts_model:
        ...
        try:
            ...
            # [OFFLINE] Built-in predefined voice (e.g. "alba", "fantine")
            if voice_info.get("type") == "builtin":
                voice_states[voice_id] = tts_model.get_state_for_audio_prompt(
                    voice_file
                )
            else:
                # Custom WAV-based voice — convert if needed
...
async def list_voices():
    """
    OpenAI-compatible voices list endpoint (enriched with metadata).
    """
    voices = []
...
async def generate_video_endpoint(request: VideoGenerateRequest):
    """
    Generate a 360p MP4 video from text + title using TTS + ffmpeg.

    Returns the MP4 file as a downloadable streaming response.
    """
    if not tts_model:
        raise HTTPException(
            status_code=503,
            detail="TTS service not available. Please install pocket_tts.",
        )

    # Validate voice
    voice_id = request.voice
    if voice_id not in available_voices and voice_id not in voice_states:
        raise HTTPException(
            status_code=400,
            detail=f"Voice '{voice_id}' not found. Available: {list(available_voices.keys())[:10]}...",
        )

    # Load voice state if not cached
    if voice_id not in voice_states:
        voice_states[voice_id] = get_voice_state(voice_id)

    voice_state = voice_states.get(voice_id)
    if voice_state is None:
        raise HTTPException(
            status_code=500,
            detail=f"Failed to load voice state for '{voice_id}'.",
        )
...
║  Health:       http://localhost:{port:<4}/health               ║
╠══════════════════════════════════════════════════════════════╣
║  OpenAI Endpoints:                                           ║
